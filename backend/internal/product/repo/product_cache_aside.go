...
//   - 详情缓存：基本信息 30 分钟，价格/库存 1 分钟（分离存储）
//   - 预热缓存：2 分钟（预测性质）
//
// 一致性保障：
...
//
// 亮点：
//	分离缓存	基本信息 30min，价格/库存 1min，独立 TTL
//	精准查询	价格/库存 miss 时只查 SELECT price, stock，减少网络传输
...
// 后续演进方向：Canal + MQ 模式（见 product.go）
// =============================================================================
...
func NewCacheAsideProductRepo(dao dao.ProductDao, cache cache.ProductCache, l logger.LoggerV1) ProductRepo {
	return &CacheAsideProductRepo{
		...
		detailPriceStockTTL: 1 * time.Minute,  // 价格/库存：1分钟（主动删除 + TTL 兜底，最多 1 分钟恢复一致）
...
func PriceStockKey(id int64) string {
	return fmt.Sprintf("product:detail:price_stock:%d", id)
}
...
func (r *CacheAsideProductRepo) GetProduct(ctx context.Context, id int64) (product domain.Product, err error) {
	...
	// 情况2：基本信息命中，价格/库存未命中（分离存储的优势）
	// 优化点：只查 price, stock 两个字段，不查完整商品
	if basicErr == nil && priceStockErr != nil {
		if err := json.Unmarshal(basicData, &product); err == nil {
...
func (r *CacheAsideProductRepo) cachePriceStock(ctx context.Context, id int64, price int64, stock int64, ttl time.Duration) {
...
func (r *CacheAsideProductRepo) deleteProductCache(ctx context.Context, id int64) {
	...
	if err := r.cache.Delete(ctx, priceStockKey); err != nil {
		r.logger.Error("删除价格/库存缓存失败", logger.Int64("product_id", id), logger.Error(err))
	}
}
