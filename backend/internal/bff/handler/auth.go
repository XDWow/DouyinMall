...
// Refresh POST /auth/refresh — 用 refresh token 换新的 token pair
func (h *AuthHandler) Refresh(c *gin.Context) {
	var req refreshReq
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, ginx.Result{Code: 4, Msg: "参数错误"})
		return
	}
