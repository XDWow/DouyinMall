...
func NewMCPRegistry(ctx context.Context, servers []agentconfig.MCPServerConfig) (*Registry, error) {
	...
	tools := make([]einotool.BaseTool, 0, len(discovered))
	invokables := make(map[string]registeredInvokableTool, len(discovered))
	for _, item := range discovered {
		tools = append(tools, item.Tool)
		invokables[item.Tool.info.Name] = registeredInvokableTool{
			invokable: item.Tool,
...
func (t *mcpWrappedTool) Info(ctx context.Context) (*schema.ToolInfo, error) {
	return t.info, nil
}
