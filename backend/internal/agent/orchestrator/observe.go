...
func appendTraceStep(flow *FlowContext, node, status string, d time.Duration, detail string) {
	if flow == nil {
		return
	}
...
func sendEvent(ctx context.Context, writer StreamWriter, event string, data any) {
	if writer == nil {
		return
	}
